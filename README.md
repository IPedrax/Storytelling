...
- **Keeps notes organic.** One entry note per character or world, growing until a section genuinely outgrows the page and earns its own file. No template folders full of blank `Relationships.md` files waiting to shame you.
...
Notes live in plain markdown in an Obsidian vault you own. Every world and every character is a folder that is *also its own vault*, so any one of them opens standalone — while the root vault sees and links across all of them at once.
...
│   └── C-Elara/            a character of that world
│       ├── .obsidian/
│       └── C-Elara.md
└── C-Wanderer/             a character with no world yet
    ├── .obsidian/
    └── C-Wanderer.md
```

`W-` is a world, `C-` is a character, and a character belonging to a world lives inside it. On first run the skill asks where your vault is and records the answer, so it only asks once. Obsidian is optional — it's markdown in folders either way.
